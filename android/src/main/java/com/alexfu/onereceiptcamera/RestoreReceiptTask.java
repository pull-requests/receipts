...
public class RestoreReceiptTask extends AsyncTask<ArrayList<Receipt>, Void, Void> {

    private Context mContext;

    public RestoreReceiptTask(Context context) {
        mContext = context;
    }
    ...
    protected Void doInBackground(ArrayList<Receipt>... receipts) {
        ...
        for(int i = 0, len = receipts[0].size(); i < len; i++) {
            dbManager.restoreReceipt(receipts[0].get(i).getId());
        }
        return null;
    }
}
